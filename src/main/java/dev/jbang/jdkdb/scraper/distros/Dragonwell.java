...
public class Dragonwell extends GitHubReleaseScraper {
	private static final String VENDOR = "alibaba";
	private static final String DISTRO = "dragonwell";
	...
	private JdkMetadata processAsset(JsonNode release, JsonNode asset) {
		...
		return JdkMetadata.create()
				...
				.setJavaVersion(parsed.javaVersion != null ? parsed.javaVersion : parsed.version)
				.setJvmImpl("hotspot")
				.setOs(normalizeOs(parsed.os))
				.setArchitecture(normalizeArch(parsed.arch))
				.setFileType(normalizeFileType(parsed.ext))
				.setImageType("jdk")
